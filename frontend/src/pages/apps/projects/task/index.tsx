...
import { tasksList, taskTable, widgetsData } from "@src/data";
import {
  Dropdown,
  DropdownButton,
  DropdownMenu,
...
const TaskTable = ({ tasks, itemsPerPage, currentPage }: any) => {
  ...
  return (
    <React.Fragment>
      <table className="table flush whitespace-nowrap">
        ...
        <tbody>
          {paginatedTasks.map(
            (project: any, index: React.Key | null | undefined) => (
              <tr key={index}>
                ...
                <td>
                  <div className="flex ml-3 -space-x-3 grow">
                    {(project.assignees || []).map(
                      (assignee: string, idx: React.Key | null | undefined) => (
                        <Link
                          key={idx}
                          to="#!"
...
const ProjectsTask: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    document.title = "Task | Domiex - React TS Admin & Dashboard Template";
  }, []);
                ...
                <div className="card" key={idx}>
                  <div className="card-body">
                    <div className="relative">
                      <div
                        className={`absolute top-0 z-0 size-11 blur-md ${item.bgClass}`}
                      ></div>
                      {IconComponent && (
                        <IconComponent
