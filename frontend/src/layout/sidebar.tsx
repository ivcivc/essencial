...
const Sidebar = ({
  ...
  const getLucideIcon = (icon: string, className: string) => {
    const icons: { [key: string]: React.ReactElement } = {
      ...
      "calendar-days": <Calendar className={className} />,
  ...
  const isActive = (menuItem: any) => {
    ...
    return menuItem.children.some(
      (child: any) =>
        ...
        (child.children && child.children.length > 0 && isActive(child)), // Recursively check nested children
    );
  };
                  ...
                  >
                    <div className="group-data-[sidebar=small]:hidden">
                      <img
                        ...
                        aria-label="logo"
                        alt="logo"
                        className="h-[43px] mx-auto group-data-[sidebar-colors=light]:dark:hidden group-data-[sidebar-colors=dark]:hidden group-data-[sidebar-colors=brand]:hidden group-data-[sidebar-colors=purple]:hidden group-data-[sidebar-colors=sky]:hidden"
                        width={132}
                        height={43}
                      />
                      <img
                        ...
                        aria-label="logo"
                        alt="logo"
                        className="h-[43px] mx-auto group-data-[sidebar-colors=light]:hidden group-data-[sidebar-colors=light]:dark:inline-block"
                        width={132}
                        height={43}
                      />
                    </div>
                    ...
                    >
                      <DropdownButton colorClass="flex items-center w-full gap-2 p-4 text-left group-data-[sidebar=small]:px-0">
                        ...
                        <div className="grow group-data-[sidebar=icon]:hidden group-data-[sidebar=small]:hidden overflow-hidden text-new-500">
                          ...
                          <p className="text-menu-title text-14">{user?.role === 'admin' ? 'Administrador' : 'Recepcionista'}</p>
                        </div>
                      ...
                      <DropdownMenu menuclass="z-50 p-5 bg-white rounded-md shadow-lg !w-64 !left-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <div>
                            <h6>Olá, {user?.name?.split(' ')[0] || 'Usuário'}</h6>
                            <p>
                ...
                >
                  {searchSidebar && searchSidebar.length > 0
                    ? searchSidebar.map((item: any, index: number) => (
                        ...
                        >
                          {/* Check for separator */}
                          {(!item.children || !item.children.length) && item.separator && (
                          ...
                          {/* If it has children */}
                          {!item.separator && item.children && item.children.length > 0 && (
                            <Dropdown
